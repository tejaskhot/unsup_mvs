...
def get_file_list(path):
    '''
    ignore files that start with something like ._
    '''
...
def mvsnet_to_gipuma(image_folder, cam_folder, depth_folder, gipuma_point_folder):

    image_folder = os.path.join(image_folder, 'images')
    cam_folder = os.path.join(cam_folder, 'cams')
    depth_folder = os.path.join(depth_folder, 'depths_mvsnet')
    ...
    # convert cameras
    image_names = get_file_list(image_folder)
...
def probability_filter(image_folder, depth_folder, prob_threshold):
    image_folder = os.path.join(image_folder, 'images')
    depth_folder = os.path.join(depth_folder, 'depths_mvsnet')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_folder', type=str, default = '/home/tejas/unsup_mvs/outputs/')
    parser.add_argument('--image_folder', type=str, default = '/home/tejas/unsup_mvs/data/mvs_testing/dtu')
    parser.add_argument('--cam_folder', type=str, default = '/home/tejas/unsup_mvs/data/mvs_testing/dtu')
    parser.add_argument('--fusibile_exe_path', type=str, default = '/usr/home/tejas/git_repos/fusibile/fusibile')
    ...
    for model in get_file_list(model_folder):
        scans = get_file_list(os.path.join(model_folder, model))
        for scan in scans:
            image_folder = os.path.join(args.image_folder, scan)
            cam_folder = os.path.join(args.cam_folder, scan)
            depth_folder = os.path.join(model_folder, model, scan)

            # create an output folder where things get written
            point_folder = os.path.join(depth_folder, 'points_mvsnet')
            if not os.path.isdir(point_folder):
                os.mkdir(point_folder)

            # probability filter
            print ('filter depth map with probability map')
            probability_filter(image_folder, depth_folder, prob_threshold)

            # convert to gipuma format
            print ('Convert mvsnet output to gipuma input')
            mvsnet_to_gipuma(image_folder, cam_folder, depth_folder, point_folder)
